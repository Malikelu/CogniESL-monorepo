...
function patchDomToPptxBundle(bundleCode, layoutName, layout) {
    ...
    // Try to patch pptx.layout = 'LAYOUT_16x9';
    ...
    // Try to patch const PPTX_WIDTH_IN = ... ; const PPTX_HEIGHT_IN = ...;
    // Handle both old format (const PPTX_WIDTH_IN = 10;) and new format (= globalOptions._slideWidth || 10;)
    patchedCode = patchedCode.replace(
        /const PPTX_WIDTH_IN\s*=\s*(?:globalOptions\._slideWidth\s*\|\|\s*)?10;\s*const PPTX_HEIGHT_IN\s*=\s*(?:globalOptions\._slideHeight\s*\|\|\s*)?5\.625;/,
        () => {
    ...
    // If patterns don't match, try a more lenient approach
    if (!patchedLayoutName) {
        // Just look for pptx.layout = anything
        patchedCode = patchedCode.replace(
            /pptx\.layout\s*=\s*['"][^'"]+['"];/,
            defineLayoutCode
        );
    ...
    if (!patchedLayoutSize) {
        // Just look for any const PPTX_WIDTH_IN and const PPTX_HEIGHT_IN assignment
        patchedCode = patchedCode.replace(
            /const PPTX_WIDTH_IN\s*=\s*[^;]+;\s*const PPTX_HEIGHT_IN\s*=\s*[^;]+;/,
            () => {
                patchedLayoutSize = true;
                return `const PPTX_WIDTH_IN = ${layout.width};\n        const PPTX_HEIGHT_IN = ${layout.height};`;
            }
